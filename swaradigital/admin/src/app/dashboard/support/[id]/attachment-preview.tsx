...
export default function AttachmentPreview({ url }: AttachmentPreviewProps) {
  ...
  if (!isImage(url) || hasError) {
    return (
        <a 
            ...
            className="flex items-center gap-2 text-xs bg-gray-100 p-2 rounded hover:bg-gray-200 transition-colors text-gray-700"
        >
  ...
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className="block max-w-[200px] group/img relative overflow-hidden rounded-md border border-gray-200">
      <img 
        src={url} 
        alt="Attachment" 
