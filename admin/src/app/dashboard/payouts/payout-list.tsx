    ...
    return (
        <div className="space-y-4">
                            ...
                            <TableRow key={req.id} className="border-white/5 hover:bg-white/[0.02] transition-colors group">
                                ...
                                <TableCell>
                                     <Dialog>
                                        ...
                                        {/* Dialog Content Details Snippet */}
            ...
            <div className="md:hidden space-y-4">
                {requests.map((req) => (
                    <div key={req.id} className="bg-white/[0.03] backdrop-blur-xl p-4 rounded-xl border border-white/5 space-y-4">
                        <div className="flex items-start justify-between">
                            <div className="flex flex-col">
                                <span className="font-bold text-white text-base">{req.profiles?.full_name || 'Unknown'}</span>
                                <span className="text-xs text-zinc-500 font-mono mb-2">{req.profiles?.email}</span>
                                <span className="text-[10px] text-zinc-600 font-mono">{new Date(req.created_at).toLocaleDateString()}</span>
                            </div>
                            <div className="flex flex-col items-end">
                                <div className="font-black text-xl tracking-tight bg-gradient-to-r from-emerald-400 to-emerald-600 bg-clip-text text-transparent mb-2">
                                    ${req.amount.toFixed(2)}
                                </div>
                                <span className={`px-2 py-0.5 rounded-full text-[9px] uppercase font-black tracking-wider border
                                    ${req.status === 'approved' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 
                                      req.status === 'rejected' ? 'bg-red-500/10 text-red-400 border-red-500/20' : 
                                      'bg-amber-500/10 text-amber-400 border-amber-500/20'}`}>
                                    {req.status}
                        ...
                        <div className="py-3 border-y border-white/5">
                             <Dialog>
                                <DialogTrigger asChild>
                                    <Button variant="secondary" className="w-full h-10 rounded-lg bg-white/5 text-zinc-300 hover:text-white hover:bg-white/10 border border-white/5 gap-2">
                                        {req.payment_mode === 'paypal' ? <Wallet className="h-4 w-4 text-blue-400" /> : 
                                         req.payment_mode === 'upi' ? <CreditCard className="h-4 w-4 text-orange-400" /> : 
                                         <Landmark className="h-4 w-4 text-emerald-400" />}
                                        <span className="text-[11px] font-bold uppercase tracking-wider">
                                            View {req.payment_mode === 'paypal' ? 'PayPal' : req.payment_mode === 'upi' ? 'UPI' : 'Bank'} Details
                                        </span>
                                    </Button>
                                </DialogTrigger>
                                <DialogContent className="bg-zinc-950/95 backdrop-blur-xl border-white/10 w-[95vw] max-w-md rounded-2xl">
                                    <DialogHeader>
                                        <DialogTitle className="text-white font-black uppercase tracking-tight flex items-center gap-2">
                                            {req.payment_mode === 'paypal' ? <Wallet className="h-5 w-5 text-blue-400" /> : 
                                             req.payment_mode === 'upi' ? <CreditCard className="h-5 w-5 text-orange-400" /> : 
                                    ...
                                    <div className="grid gap-3 text-sm pt-4">
                                        {req.payment_mode === 'paypal' ? (
                                             <div className="flex flex-col p-3 rounded-lg bg-white/[0.03] border border-white/5 relative">
                                                <span className="text-[10px] uppercase tracking-wider text-zinc-500 font-bold mb-1">PayPal Email</span>
                                                <span className="text-white font-bold text-sm">{req.profiles?.paypal_email || 'Not provided'}</span>
                                                <Button variant="ghost" size="icon" className="absolute right-2 top-1/2 -translate-y-1/2 text-zinc-500" onClick={() => copyToClipboard(req.profiles?.paypal_email, 'PayPal Email')}>
                                                    <Copy className="h-4 w-4" />
                                                </Button>
                                            </div>
                                        ) : req.payment_mode === 'upi' ? (
                                            <div className="flex flex-col p-3 rounded-lg bg-white/[0.03] border border-white/5 relative">
                                                <span className="text-[10px] uppercase tracking-wider text-zinc-500 font-bold mb-1">UPI ID</span>
                                                <span className="text-white font-bold text-sm">{req.profiles?.upi_id || 'Not provided'}</span>
                                                <Button variant="ghost" size="icon" className="absolute right-2 top-1/2 -translate-y-1/2 text-zinc-500" onClick={() => copyToClipboard(req.profiles?.upi_id, 'UPI ID')}>
                                                    <Copy className="h-4 w-4" />
                                                </Button>
                                            </div>
                                        ) : (
                                            <>
                                                <div className="flex flex-col p-3 rounded-lg bg-white/[0.03] border border-white/5 relative">
                                                    <span className="text-[10px] uppercase tracking-wider text-zinc-500 font-bold mb-1">Bank Name</span>
                                                    <span className="text-white font-bold text-sm">{req.profiles?.bank_name || 'N/A'}</span>
                                                    <Button variant="ghost" size="icon" className="absolute right-2 top-1/2 -translate-y-1/2 text-zinc-500" onClick={() => copyToClipboard(req.profiles?.bank_name, 'Bank Name')}>
                                                        <Copy className="h-4 w-4" />
                                                    </Button>
                                                </div>
                                                <div className="flex flex-col p-3 rounded-lg bg-white/[0.03] border border-white/5 relative">
                                                    <span className="text-[10px] uppercase tracking-wider text-zinc-500 font-bold mb-1">Account No</span>
                                                    <span className="text-white font-mono font-bold tracking-wider">{req.profiles?.account_number || 'N/A'}</span>
                                                    <Button variant="ghost" size="icon" className="absolute right-2 top-1/2 -translate-y-1/2 text-zinc-500" onClick={() => copyToClipboard(req.profiles?.account_number, 'Account Number')}>
                                                        <Copy className="h-4 w-4" />
                                                    </Button>
                                                </div>
                                                <div className="flex flex-col p-3 rounded-lg bg-white/[0.03] border border-white/5 relative">
                                                    <span className="text-[10px] uppercase tracking-wider text-zinc-500 font-bold mb-1">IFSC Code</span>
                                                    <span className="text-white font-mono font-bold tracking-wider">{req.profiles?.ifsc_code || 'N/A'}</span>
                                                    <Button variant="ghost" size="icon" className="absolute right-2 top-1/2 -translate-y-1/2 text-zinc-500" onClick={() => copyToClipboard(req.profiles?.ifsc_code, 'IFSC Code')}>
                                                        <Copy className="h-4 w-4" />
                                                    </Button>
                                                </div>
                                            </>
                        ...
                        {req.status === 'pending' && (
                            <div className="flex gap-2 pt-1">
                                <Button 
                                    className="flex-1 bg-emerald-500 text-white hover:bg-emerald-600 h-10 rounded-xl font-bold shadow-lg shadow-emerald-500/20 active:scale-95 transition-transform"
                                    onClick={() => handleApprove(req.id)}
                                    disabled={loading === req.id}
                                >
                                ...
                                <Button 
                                    variant="destructive"
                                    className="w-12 h-10 rounded-xl font-bold bg-zinc-800 hover:bg-red-600 text-zinc-400 hover:text-white transition-colors active:scale-95"
                                    onClick={() => handleReject(req.id)}
                                    disabled={loading === req.id}
                                >
                                    <X size={16} />
                        ...
                        {req.status !== 'pending' && (
                            <div className="text-center pt-1 text-xs text-zinc-500 font-mono">
                                {loading === req.id ? 'Processing...' : `Completed on ${new Date(req.updated_at).toLocaleDateString()}`}
                            </div>
                        )}
                    </div>
                ))}
                ...
                {requests.length === 0 && (
                     <div className="flex flex-col items-center justify-center p-8 border border-white/5 rounded-2xl bg-white/[0.02]">
                         <DollarSign className="h-8 w-8 text-zinc-600 mb-3" />
                         <p className="text-zinc-400 font-medium">No payout requests found.</p>
                     </div>
                )}
            </div>
        </div>
